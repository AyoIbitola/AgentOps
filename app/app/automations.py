...
def format_timeline_for_slack(incident_id: str, items: list) -> str:
    ...
    lines = [f"*📜 Timeline for incident `{incident_id}`*"]
    for it in items:
        ...
        lines.append(f"- [{ts}] *{action}* → `{short}`")
    return "\n".join(lines)
def _build_email_html(
    ...
    for it in items:
        ...
        if len(short) > 120:
            short = short[:120] + "…"
        li_html.append(f"<li><b>{ts}</b> — <code>{act}</code> — <span>{short}</span></li>")
    ...
    if incident_id:
        payload["blocks"] = [
            ...
            {"type": "actions", "elements": [
                {"type": "button", "text": {"type": "plain_text", "text": "🔄 Restart Service"},
                 "value": json.dumps({
                     "incident_id": incident_id,
                ...
                {"type": "button", "text": {"type": "plain_text", "text": "📜 View Timeline"},
